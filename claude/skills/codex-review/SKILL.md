...
3. `@codex-reviewer` subagent を呼び出してレビューを Codex に委譲する
   - `$ARGUMENTS` が指定されている場合はファイルパスとして扱い、`TARGET_PATH=$ARGUMENTS` を渡す
   - `$ARGUMENTS` が未指定の場合はブランチ全体を対象にする
4. subagent の結果をユーザーに提示する
5. 指摘がある場合、修正するかどうかをユーザーに確認する

## 重要

- 自分自身でレビューを行わないこと。必ず `@codex-reviewer` に委譲する
- 生ファイルは `${TMPDIR:-/tmp}/codex-review-latest.md` に保存される
